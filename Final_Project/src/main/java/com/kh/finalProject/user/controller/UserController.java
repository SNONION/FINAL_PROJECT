...
public class UserController {
	...
	public AddressInfo getLocationName(String latitude, String longitude) throws Exception {
		...
		new Location();
		Location locationInfo = Location.builder().locationName(addressName.get("region_1depth_name").getAsString())
				.locationDetail1(addressName.get("region_2depth_name").getAsString())
				.locationDetail2(addressName.get("region_3depth_name").getAsString()).build();
	...
	public ModelAndView loginUser(User user, String saveId, HttpServletResponse response, HttpSession session,
	...
	public HashMap<String, String> purchaseKakao(int payMount, HttpServletRequest request) throws Exception {
		...
		String jsonData = String.format(
				"{" + "\"cid\": \"TC0ONETIME\"," + "\"partner_order_id\": \"%s\"," + "\"partner_user_id\": \"%s\","
						+ "\"item_name\": \"kakao\"," + "\"quantity\": \"1\"," + "\"total_amount\": \"%d\","
						+ "\"vat_amount\": \"%d\"," + "\"tax_free_amount\": \"0\","
	...
	public String approvalProcess(String pgToken, String tid, String partnerOderId, HttpServletRequest request)
			throws Exception {
		...
		// JSON 데이터 준비
		String jsonData = String.format(
				"{" + "\"cid\": \"TC0ONETIME\"," + "\"tid\": \"%s\"," + "\"partner_order_id\": \"%s\","
						+ "\"partner_user_id\": \"%s\"," + "\"pg_token\": \"%s\"," + "}",
				tid, // tid
				partnerOderId, // partner_order_id
				user.getUserId(), // partner_user_id
				pgToken // pg_token
	...
	public ModelAndView changeProfileImg(MultipartFile upfile, HttpServletRequest request, ModelAndView mv)
			throws Exception {
