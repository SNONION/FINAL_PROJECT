...
<body>
	...
	<div class="outer">
		...
		<div class="container" id="notice-container">
			<c:if test="${loginUser.userId == 'admin' && loginUser != null}">
				<button class="btn-kakao" style="float: left; margin-bottom: 10px;" onclick="writeNotice();">글작성</button>
			</c:if>
			<table class="table" id="notice-table">
				...
				<tbody id="notice-click">
					<c:forEach var="i" items="${nList}">
						<tr align="center">
							<td>${i.noticeNo}</td>
							<td id="titleBar-area">${i.noticeTitle}</td>
			...
			<div class="paging-area">
				<ul class="pagination">
					<c:if test="${pi.currentPage > 1}">
						<li class="page-item"><a class="page-link" href="${contextPath}/board/toNotice?currentPage=${pi.currentPage - 1}"><</a></li>
					</c:if>
					<c:forEach var="i" begin="${pi.startPage}" end="${pi.maxPage}">
						<li class="page-item"><a class="page-link" href="${contextPath}/board/toNotice?currentPage=${i}">${i}</a></li>
					</c:forEach>
	...
	<script>
		$("#notice-click").on("click","tr",function(){
			...
			var noticeNo = $(this).children().first().text();
			
			location.href="${contextPath}/board/toNoticeDetail?noticeNo=" + noticeNo;
			
		});
		
		function writeNotice(){
			
			location.href="${contextPath}/board/toNoticeEnroll";
		}
	</script>
